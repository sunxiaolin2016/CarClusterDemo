 ...
 */
public class SampleClusterServiceImpl extends Service {
    ...
    public IBinder onBind(Intent intent) {
        ...
        //return (LOCAL_BINDING_ACTION.equals(intent.getAction())) ? mLocalBinder : super.onBind(intent);
        return (LOCAL_BINDING_ACTION.equals(intent.getAction())) ? mLocalBinder : null;
    }
...
//                CarNavigationInstrumentCluster config = CarNavigationInstrumentCluster.createCluster(1000);
    ...
    private void execShellCommand(String[] args) {
        ...
        switch (command) {
            ...
            case "destroyOverlayDisplay": {
                //Settings.Global.putString(getContentResolver(), Global.OVERLAY_DISPLAY_DEVICES, "");
                break;
            }

            case "createOverlayDisplay": {
                if (args.length > 1) {
                    //Settings.Global.putString(getContentResolver(), Global.OVERLAY_DISPLAY_DEVICES, args[1]);
                } else {
...
//                    try {
//                        setClusterActivityState(args[1],  ClusterActivityState.create(true, unobscuredArea).toBundle());
